...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(x.size(1)):
            y = self.layer1(x[:, i, :]) # в y сохраняем результат шага
            y += h # складываем с предыдущим
            y = torch.tanh(y) # активация
            h = y # сохраняем для следующего шага
        y = self.layer2(y)
        y = torch.sigmoid(y)
        return y
...
RNN(
    input_size=33,  # размер данных (отдельные элементы) для входного тензора 33 в случе кириллицы, если каждая буква это [1, 0, 0 .....]
    hidden_size=64,  # размер вектора скрытого состояния
    num_layers=1,  # количество последовательных рекуррентных слоев
...
# batch_size = 8 количество наборов токенов, размер батча, количество букв, слов по количеству которого мы должны угадать следующую
...
h0 = torch.randn(1, 8, 64) # 1 так как 1 элемент h0, 8 - для 8 батчей, 64 - размер скрытого состояния
...
print(y.size())  # torch.Size([8, 3, 64]) все выходы с каждой итерации, так как 3 итерации
print(h.size())  # torch.Size([1, 8, 64]) если слоев (num_layers) > 1 то на выходе будет [n, 8, 64]
```
...
# y : [8, 3, 32] - 32 так как объединение прямого и обратного прохода
