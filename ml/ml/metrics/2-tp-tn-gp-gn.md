...
Модель не выдаёт метку напрямую — она выдаёт скор $b(x) = \langle \omega, x \rangle$: вещественное число, чем выше — тем больше модель уверена в принадлежности к положительному классу. Метка получается через **порог** $t$: $a(x) = \text{sign}(b(x) - t)$. При $b(x) > t$ объект относится к классу $+1$, иначе — к $-1$. Стандартный выбор — $t = 0$, но его можно сдвигать: повышение $t$ делает классификатор осторожнее (реже говорит «+1»), снижение — агрессивнее. Именно от выбора $t$ зависит, в какую из четырёх ячеек попадёт каждый объект.
...
![Матрица ошибок и пример со спам-фильтром](images/confusion-matrix.png)
...
![Кривая Precision–Recall и зависимость F_β от β](images/pr-fmeasure.png)
