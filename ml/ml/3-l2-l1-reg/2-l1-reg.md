...
Функция потерь дополняется штрафом на модуль весов $L_i^{reg}(ω) = L_i(ω) + λ ||ω|| = L_i(ω) + λ \sum_{i=0}^{n}|ω_i|$. Так как модуль не дифференцируем в нуле, используется субградиент: $∇L_i^{reg}(ω) = ∇L_i(ω) + λ * Sign(ω)$, где $Sign(ω)$ — поэлементная функция знака.

Тогда стохастический градиентный спуск: $ω_{t + 1} = ω_t - η * (∂L(ω, x_i, y_i) / ∂ω + λ * Sign(ω_t))$.

L1 регуляризация приводит к разреженности весов (часть коэффициентов становится равной нулю), что позволяет отбирать наиболее важные признаки.
