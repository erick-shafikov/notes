...
Из известных ядер можно строить новые. Если $K_1, K_2$ — ядра, то ядрами также являются:

- скалярное произведение: $K(x, x') = \langle x, x' \rangle$
- константа: $K(x, x') = c > 0$
- произведение: $K(x, x') = K_1(x, x') \cdot K_2(x, x')$
- применение функции: $K(x, x') = f(K_1(x, x'))$ для подходящих $f$
- взвешенная сумма: $K(x, x') = \alpha_1 K_1(x, x') + \alpha_2 K_2(x, x')$ при $\alpha_1, \alpha_2 > 0$
- явное отображение: $K(x, x') = \langle \psi(x), \psi(x') \rangle$ для произвольного $\psi: \mathcal{X} \to \mathbb{R}^n$

Важно: для каждого конкретного ядра вычислительная схема оптимизации SVM меняется — единого функционала для произвольных ядер не существует (если только не переходить к субградиентным методам).
...
- Нет встроенного механизма отбора признаков (аналог L1-регуляризатора). Необходимо подбирать гиперпараметры: параметр регуляризации $C$, параметр ядра $\gamma$ (для RBF) и другие.
- Вычислительная сложность обучения $O(n^2)$–$O(n^3)$ по числу объектов — плохо масштабируется на большие датасеты.

SVM считается лучшим методом бинарной классификации среди линейных классификаторов. При необходимости можно дополнительно добавить L1-регуляризатор для отбора признаков.
