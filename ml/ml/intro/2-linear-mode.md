...
Решающая функция имеет вид $a(x) = \text{sign}(\langle w, x \rangle - w_0)$. Введём понятие отступа $M_i = y_i (\langle w, x_i \rangle -w_0)$: если $M_i > 0$ — классификация верная, если $M_i < 0$ — ошибка. Тогда функция потерь может быть записана как $L(a, y) = [y \cdot a(x) < 0]$, а показатель качества $Q(w) = \sum*{i=1}^{n} [M_i < 0]$.
