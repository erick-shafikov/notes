...
Если рассмотреть $ω_t$ и $ω_{t + 1}$ в пространстве параметров (например, в 2D) ![l1 vs l2](images/l1-l2.png), то геометрически L1 регуляризатор задает область в виде ромба, а L2 — в виде окружности. Рассмотрим оптимальное решение $\tilde{ω}$, тогда вероятность того, что решение попадет в вершину (угол) у L1 выше, чем у L2, что приводит к занулению некоторых координат.
