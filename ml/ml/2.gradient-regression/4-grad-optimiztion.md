...
Метод используется для ускорения градиентного спуска и сглаживания колебаний. Идея в том, что к текущему шагу добавляется часть предыдущего шага, за счет чего накапливается “инерция” движения и алгоритм быстрее движется в одном направлении и меньше колеблется Формула итерации: $v_{n+1} = β * v_n + grad f(x_n)$, $x_{n+1} = x_n - η * v_{n+1}$, где $v$ — накопленный вектор скорости (импульса), $β$ — коэффициент импульса (обычно 0.9), $η$ — шаг обучения. Смысл в том, что градиенты накапливаются во времени, и если направление сохраняется, шаги ускоряются, а если градиенты меняют знак, колебания сглаживаются. Экспоненциальное скользящее среднее градиента. Метод позволяет быстрее сходиться по “пологим” направлениям и уменьшает колебания в “крутых” направлениях по сравнению с обычным градиентным спуском.
