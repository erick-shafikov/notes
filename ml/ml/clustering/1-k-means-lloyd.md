...
**Числовой пример.** Шесть точек в двух компактных группах:
...
Выберем $K = 2$ и намеренно «неудачные» начальные центры: $\mu_1 = (0, 0)$, $\mu_2 = (2, 0)$ — оба лежат в области первой группы.

**Итерация 1. Назначение** (расстояния квадратичные $\|x_i - \mu_a\|^2$):

| Точка     | $\rho^2$ до $\mu_1$ | $\rho^2$ до $\mu_2$ | Кластер |
| --------- | ------------------- | ------------------- | ------- |
| A $(0,0)$ | $0$                 | $4$                 | $C_1$   |
| B $(2,0)$ | $4$                 | $0$                 | $C_2$   |
| C $(0,2)$ | $4$                 | $8$                 | $C_1$   |
...
Результат: $C_1 = \{A, C\}$, $C_2 = \{B, D, E, F\}$. Точка B попала в $C_2$ ошибочно — центр $\mu_2$ стоял прямо на ней.

**Итерация 1. Обновление центров:**

$$\mu_1 = \frac{(0,0)+(0,2)}{2} = (0,\, 1), \qquad \mu_2 = \frac{(2,0)+(6,6)+(8,6)+(6,8)}{4} = (5.5,\, 5)$$

**Итерация 2. Назначение:**

| Точка     | $\rho^2$ до $\mu_1=(0,1)$ | $\rho^2$ до $\mu_2=(5.5,5)$ | Кластер |
| --------- | ------------------------- | --------------------------- | ------- |
| A $(0,0)$ | $1$                       | $55.25$                     | $C_1$   |
| B $(2,0)$ | $5$                       | $37.25$                     | $C_1$   |
| C $(0,2)$ | $1$                       | $39.25$                     | $C_1$   |
...
Точка B переходит в $C_1$: $C_1 = \{A, B, C\}$, $C_2 = \{D, E, F\}$.

**Итерация 2. Обновление центров:**

$$\mu_1 = \frac{(0,0)+(2,0)+(0,2)}{3} = \!\left(\tfrac{2}{3},\, \tfrac{2}{3}\right), \qquad \mu_2 = \frac{(6,6)+(8,6)+(6,8)}{3} = \!\left(\tfrac{20}{3},\, \tfrac{20}{3}\right)$$

**Итерация 3.** Назначения не меняются — алгоритм сошёлся. Финальные кластеры совпали с истинными группами, несмотря на неудачную инициализацию.
