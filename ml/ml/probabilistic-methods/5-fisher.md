...
В общем гауссовском байесовском классификаторе у каждого класса своя ковариационная матрица $\Sigma_y$ — граница между классами получается **квадратичной** (квадратичный дискриминантный анализ, QDA). Это мощно, но дорого: нужно оценить $K$ матриц размером $n \times n$, каждая из которых требует $O(n^2)$ параметров — при малой выборке они оцениваются плохо и модель переобучается.

Если предположить, что все классы имеют **одинаковую** $\Sigma$, квадратичные слагаемые сокращаются и граница становится **линейной**. Это линейный дискриминантный анализ (LDA / дискриминант Фишера): меньше параметров, устойчивее оценки, быстрее работает.

**Когда одинаковая $\Sigma$ разумна.** Это допущение оправдано, когда классы отличаются только центрами, но имеют схожую внутреннюю структуру разброса:

- Один и тот же физический процесс с разными условиями — например, измерения одного прибора при разных температурах: шум прибора (форма облака) одинаков, смещается только центр.
- Классы порождены одним источником вариабельности — например, рост людей разных национальностей: дисперсия внутри группы схожа, различаются только средние.
- Малая выборка — нет достаточно данных, чтобы надёжно оценить отдельную $\Sigma_y$ для каждого класса; общая $\Sigma$ по всей выборке оценивается устойчивее.

Когда облака классов имеют очень разную форму или ориентацию — допущение нарушается и QDA предпочтительнее.
...
Поскольку $\Sigma$ одинакова для всех классов, слагаемое $\frac{1}{2}\ln\det\Sigma$ не зависит от $y$ и выпадает из $\arg\max$. Раскрываем квадратичную форму, обозначив $A = \Sigma^{-1}$ для краткости:

$$-(x - \mu_y)^T A\,(x - \mu_y)$$

Раскрываем скобки как обычное произведение, пользуясь линейностью транспонирования:

$$(x - \mu_y)^T A\,(x - \mu_y) = (x^T - \mu_y^T)\,A\,(x - \mu_y)$$
$$= x^T A\,(x - \mu_y) - \mu_y^T A\,(x - \mu_y)$$
$$= x^T A x - x^T A \mu_y - \mu_y^T A x + \mu_y^T A \mu_y$$

Поскольку $A = \Sigma^{-1}$ симметрична, скаляр $\mu_y^T A x$ равен своему транспонированному $x^T A \mu_y$, поэтому два средних члена одинаковы:

$$= x^T A x - 2\,x^T A \mu_y + \mu_y^T A \mu_y$$

Умножаем на $-\frac{1}{2}$ и возвращаем $A = \Sigma^{-1}$:
