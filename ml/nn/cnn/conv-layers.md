...
При работе с изображениями применяются свёрточные нейронные сети (Convolution Neural Network). Вместо полносвязного слоя, где каждый нейрон смотрит на весь входной вектор, здесь нейрон смотрит только на небольшую локальную область — прямоугольное окно размером $F \times F$, которое скользит по изображению. Веса этого окна называют **фильтром** или **ядром свёртки**.

## Операция свёртки (одноканальный случай)

Двумерное входное изображение $H \times W$ пропускается через фильтр размером $F \times F$. Фильтр накладывается на каждую позицию $(n, m)$ изображения и вычисляет взвешенную сумму покрытых пикселей:

$$z_{nm} = \omega_0 + \sum_{i=1}^{F}\sum_{j=1}^{F} \omega_{ij} \cdot x_{n+i-1,\; m+j-1}$$

где $\omega_{ij}$ — веса фильтра (обучаемые параметры), $\omega_0$ — bias. Затем применяется функция активации:

$$u_{nm} = \sigma(z_{nm})$$

Совокупность значений $u_{nm}$ для всех позиций образует **карту признаков** (feature map). Размер карты признаков при нулевом padding: $(H - F + 1) \times (W - F + 1)$. Разные фильтры извлекают разные паттерны: фильтр с вертикальными весами выделяет вертикальные границы, горизонтальный — горизонтальные линии.

## Многоканальная свёртка

Если входное изображение имеет $C$ каналов (например, RGB: $C = 3$), фильтр становится трёхмерным: $C \times F \times F$. Свёртка суммирует вклады по всем каналам:

$$z_{nm} = \omega_0 + \sum_{k=1}^{C}\sum_{i=1}^{F}\sum_{j=1}^{F} \omega_{k,i,j} \cdot x_{k,\; n+i-1,\; m+j-1}$$

где $k$ — индекс входного канала, $\omega_{k,i,j}$ — веса трёхмерного фильтра для канала $k$. Один такой фильтр порождает одну выходную карту признаков. Применяя $K$ различных фильтров параллельно, получаем $K$ карт признаков — это и есть $K$ **выходных каналов** (`out_channels` в PyTorch). Каждый последующий слой выделяет всё более сложные признаки: первый слой — края, второй — текстуры, третий — формы объектов.

![Свёрточные нейронные сети: принцип работы и пулинг](images/cnn-theory.png)

## Пулинг (Pooling)

После свёртки карта признаков покрывается непересекающимися окнами фиксированного размера. В каждом окне выбирается одно агрегированное значение:

- **MaxPooling** — максимальное значение в окне; сохраняет наиболее выраженный отклик детектора признаков.
- **MinPooling** — минимальное значение; применяется редко.
- **AveragePooling** — среднее значение; сглаживает карту признаков.

Пулинг уменьшает пространственные размеры карты (даунсэмплинг), снижая вычислительную нагрузку и делая представление инвариантным к небольшим сдвигам входного изображения.
