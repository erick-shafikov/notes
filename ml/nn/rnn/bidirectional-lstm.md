...
В RNN дальний контекст теряется: скрытое состояние $h_t$ несёт информацию только о нескольких последних шагах, потому что градиент экспоненциально затухает при обратном распространении. LSTM (Long Short-Term Memory) решает эту проблему, добавляя второй вектор состояния — **вектор контекста** $C_t$ — который течёт через всю последовательность практически без изменений, если не управлять им явно через ворота. Верхняя шина $C_t$ реализует длительную (долгосрочную) память, а $h_t$ остаётся кратковременной.

### Ворота LSTM

![LSTM ячейка](images/lstm-cell.png)

На каждом шаге все ворота принимают на вход конкатенацию предыдущего скрытого состояния и текущего входа: $[h_{t-1}, x_t]$.

**1. Ворота забывания** — решают, какую долю предыдущего контекста стереть:

$$f_t = \sigma\!\left(W_f [h_{t-1},\, x_t] + b_f\right)$$

$f_t$ — вектор значений в $(0, 1)$ той же размерности, что $C_t$; элемент, близкий к 0, означает «забыть», близкий к 1 — «сохранить».

**2. Ворота входа и кандидат** — решают, какую новую информацию добавить в контекст:

$$i_t = \sigma\!\left(W_i [h_{t-1},\, x_t] + b_i\right), \qquad C_t^* = \tanh\!\left(W_c [h_{t-1},\, x_t] + b_c\right) \in [-1, +1]$$

$i_t$ — маска, определяющая силу нового добавления; $C_t^*$ — вектор кандидатов новых значений.

**3. Обновление контекста** — поэлементные операции:

$$a_t = i_t \odot C_t^*, \qquad C_t = f_t \odot C_{t-1} + a_t$$

Старый контекст $C_{t-1}$ частично стирается через $f_t$, затем к нему добавляются новые знания $a_t$. Операции только сложение и умножение — это ключевой момент: градиент по $C$ проходит без перемножения на матрицу весов, что существенно сглаживает затухание.

**4. Ворота выхода и скрытое состояние:**

$$O_t = \sigma\!\left(W_o [h_{t-1},\, x_t] + b_o\right), \qquad h_t = O_t \odot \tanh(C_t)$$

$O_t$ контролирует, какая часть контекста «просачивается» в скрытое состояние $h_t$, которое передаётся на следующий шаг и используется как выход.

Таким образом, LSTM управляет тремя процессами через обучаемые ворота: **забывание** ненужного ($f_t$), **добавление** нового ($i_t$, $C_t^*$) и **формирование** выхода ($O_t$). Упрощённая версия этой архитектуры — GRU, объединяющая ворота забывания и входа в одно.
