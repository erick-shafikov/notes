...
RNN (Recurrent Neural Network) — рекуррентная нейронная сеть — предназначена для обработки последовательных данных: звука, текста, временных рядов, траекторий. В отличие от обычной полносвязной сети, она сохраняет скрытое состояние $h_t$, которое переносит информацию о предыдущих элементах последовательности.

В общем виде скрытое состояние на шаге $k$ определяется как:

$$h_k = f(x_k,\, h_{k-1})$$

где $f$ — некоторая нелинейная функция, $x_k$ — входной сигнал на шаге $k$. Если $f(x, h) = \alpha(x + h)$ для некоторого веса $\alpha$, то для трёх входных сигналов:

$$h_3 = \bigl(((x_1 + 0)\,\alpha + x_2)\,\alpha + x_3\bigr)\alpha = \alpha^3 x_1 + \alpha^2 x_2 + \alpha x_3$$

Видно, что более ранние входы ($x_1$) взвешены большей степенью $\alpha$: при $|\alpha| < 1$ их вклад экспоненциально затухает, а при $|\alpha| > 1$ — взрывообразно растёт. Именно в этом корень проблемы исчезающих и взрывных градиентов.

## Вычисление на каждом шаге

На каждом шаге $t$ выполняется:

$$a_t = W x_t + U h_{t-1} + b, \qquad h_t = \tanh(a_t)$$

$$O_t = V h_t + c, \qquad y_t = \mathrm{softmax}(O_t)$$

где $W$ — матрица весов вход→скрытый, $U$ — матрица скрытый→скрытый, $V$ — матрица скрытый→выход, $b, c$ — смещения. Начальное состояние $h_0 = \mathbf{0}$. Поскольку сигналы поступают строго последовательно, распараллелить вычисления невозможно — для обучения используется **BPTT (Backpropagation Through Time)**: сеть разворачивается по времени в вычислительный граф и применяется обычный обратный проход, при этом веса $W, U, V$ одинаковы на всех шагах.

## Топологии

В зависимости от задачи применяют разные режимы входов и выходов:

* **Many-to-Many (MTM)** — выход на каждом шаге: перевод текста, разметка последовательности
* **Many-to-One (MTO)** — один выход в конце: пометочная/тональная разметка текста, классификация
* **One-to-Many (OTM)** — один вход, много выходов: описание изображения (image captioning)
* **One-to-One (OTO)** — единичный вход и выход: для особых редких случаев

![Топологии RNN](images/rnn-topologies.png)

Многослойные (stacked) RNN передают скрытые состояния первого слоя как входы второго; двунаправленные (bidirectional) запускают два независимых прохода — прямой и обратный — и конкатенируют их скрытые состояния.

## Проблемы

* Рост локальных градиентов (exploding gradients)
* Угасание локальных градиентов (vanishing gradients)

Для входных данных используется **one-hot encoding**: каждый токен кодируется вектором из $\{1, 0, \ldots, 0\}$ нужной длины.

## Почему tanh, а не ReLU

Производная $\tanh$ имеет вид колоколообразной кривой с максимумом 1 в нуле и убывает к нулю на краях. Это означает, что $\tanh'(x) \in (0, 1]$ — градиент никогда не превысит 1 при обратном проходе через одну активацию. При BPTT градиент перемножается по всем шагам времени: если каждый множитель $\leq 1$, произведение убывает, но не взрывается.

ReLU имеет производную 0 или 1. При значении 1 и большой длине последовательности произведение остаётся 1, однако если в какой-либо точке скрытое состояние выходит за $[0,1]$, цепочка перемножений даёт неконтролируемый рост градиентов. Tanh, насыщая выход в $(-1, 1)$, структурно ограничивает этот рост.
