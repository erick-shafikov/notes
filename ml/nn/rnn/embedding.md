...
One-hot векторы хорошо работают для отдельных букв, но для слов и предложений не подходят: косинусное расстояние между любыми двумя разными one-hot векторами одинаково (равно нулю схожести), поэтому семантическая близость слов никак не отражается.

**Embedding** — компактное плотное векторное представление, которое сжимает смысл объекта в вектор фиксированной небольшой размерности. Близкие по смыслу слова оказываются близки в пространстве эмбеддингов. Набор методов получения таких представлений — **Word2Vec**.

### Матрица совместной встречаемости

Один из первых подходов к формированию векторного представления слов — подсчёт, как часто каждое слово встречается в контексте других слов. Строится матрица совместной встречаемости: строки — слова словаря, столбцы — контекстные слова, ячейка — число вхождений. Такой вектор-строка отражает взаимосвязи слова с остальными. Для сокращения размерности применяют PCA, SVD или нейронную сеть.

### Word2Vec: Skip-gram и CBOW

В нейросетевом подходе вводятся две матрицы эмбеддингов: $W_{em}$ (вход, размер $V \times d$) и $U_{em}$ (выход, размер $d \times V$), где $V$ — размер словаря, $d$ — размерность эмбеддинга. One-hot вектор слова умножается на $W_{em}$ и получает плотный вектор; затем умножается на $U_{em}$ и через softmax даёт вероятности контекстных слов.

**Skip-gram** — прогноз контекстных слов по целевому: для каждого слова берётся окно (например, 2 слова слева и 2 справа) и модель учится предсказывать эти слова. Слова из контекста по очереди служат целевыми метками обучения; слово, которое появляется в контексте, должно получить наибольшую вероятность.

**CBOW** (Continuous Bag of Words) — наоборот: контекстные слова подаются на вход, модель предсказывает целевое слово.

После обучения строки матрицы $W_{em}$ становятся эмбеддингами слов — именно их используют как входные признаки.

### Эмбеддинги в CNN и Transfer Learning

В сетях типа VGG16-19 последний полносвязный слой (до финального классификатора) фактически является эмбеддингом изображения: при пропуске через сеть картинок схожего содержания их представления в этом слое оказываются близки.

При transfer learning с текстом: добавляют предобученный слой `Embedding` перед основной архитектурой, замораживая или дообучая его:

```
[one-hot] → [Embedding] → [CNN / RNN] → [Linear]
```

Это позволяет использовать готовые веса (например, из navec или word2vec) вместо обучения эмбеддингов с нуля.
