## Рекуррентный слой: затухание градиентов и двунаправленный RNN

### Градиент через время (BPTT)

Скрытое состояние RNN на шаге $t$:

$$h_t = \tanh(W x_t + V h_{t-1} + b)$$

где $W$ — матрица весов для входа, $V$ — матрица рекуррентных весов. Если $\hat{y}_t = h_t$, то градиент функции потерь по $W$ через цепное правило разворачивается по всем шагам:

$$\frac{\partial L}{\partial W} = \frac{\partial L}{\partial h_t}\frac{\partial h_t}{\partial W} + \frac{\partial L}{\partial h_t}\frac{\partial h_t}{\partial h_{t-1}}\frac{\partial h_{t-1}}{\partial W} + \ldots + \frac{\partial L}{\partial h_t}\frac{\partial h_t}{\partial h_{t-1}}\cdots\frac{\partial h_2}{\partial h_1}\frac{\partial h_1}{\partial W}$$

Каждое звено цепи $\frac{\partial h_{k+1}}{\partial h_k} = \tanh'(\alpha_k) \cdot V$, поэтому вся цепочка от шага $k$ до $t$ равна $\prod_{j=k}^{t-1} \tanh'(\alpha_j) \cdot V$.

**Почему ReLU не подходит.** Производная ReLU равна 0 или 1, поэтому произведение сводится к $V^{t-k}$: при $|V| > 1$ градиент взрывообразно растёт, при $|V| < 1$ — затухает. Нет ничего, что ограничивало бы рост.

**Почему tanh лучше, но тоже не решает проблему.** С tanh полный градиент:

$$\frac{\partial L}{\partial W} = \tanh'(\alpha_0)\cdot V^1 + \tanh'(\alpha_1)\cdot V^2 + \ldots$$

Поскольку $\tanh'(\alpha) \leq 1$, каждый член убывает — при длинных последовательностях вклад ранних шагов исчезает. Сигмоида ведёт себя ещё хуже: её производная не превышает $0.25$.

Итог: скрытое состояние $h_t$ несёт в себе лишь **кратковременную память** — оно эффективно отражает только несколько последних шагов. Это мотивирует архитектуры LSTM и GRU, явно управляющие передачей информации через время.

### Двунаправленный RNN

![Двунаправленный RNN](images/bidirectional-rnn.png)

В задачах, где выход на позиции $t$ зависит от **обоих** направлений контекста (перевод, разметка тональности фразы, распознавание паронимов), запускают два независимых RNN — прямой и обратный — и конкатенируют их скрытые состояния:

- **Прямой проход**: $h_1 \to h_2 \to \ldots \to h_T$ (слева направо, обрабатывает прошлый контекст)
- **Обратный проход**: $g_T \to g_{T-1} \to \ldots \to g_1$ (справа налево, обрабатывает будущий контекст)
- **Выход на шаге $t$**: $[\,h_t\;;\;g_t\,]$ — конкатенация скрытых состояний обоих направлений

Оба прохода вычисляются независимо; их результаты объединяются перед выходным слоем. В PyTorch это `nn.RNN(..., bidirectional=True)`, при этом $h$ возвращается как тензор формы `(2, batch, hidden)` — первая строка прямого прохода, вторая обратного — и их конкатенируют вручную: `torch.cat([h[-2], h[-1]], dim=1)`.
