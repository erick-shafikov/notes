...
-[определение блочности элемента](./css-props.md/#display)

```scss
 {
  display: block; //друг за другом
  display: inline; //строчная
  display: inline-block; //элемент не будет переносится, но к нему можно применить width и height
}
```
...
свойства блочной модели – размеры width, height, padding, border, margin
...
<!-- inline элементы -  -->
<span>, <img />, <a> - нельзя менять ширину <script>
```

Вытекание за контейнер одно из свойств блочной модели регулируется с помощью свойства overflow

- [если контент больше чем контейнер](./css-props.md#overflow) свойство в зависимости от направленности письма:
- - overflow-block
- - overflow-inline
...
Контекст форматирование:

- блочный - устанавливает самый внешний блок initial block formatting context это обеспечивает тег html, flats будут примены только к элементам которые будут находится в одно БКФ. Помимо html БКФ создают float-ы, позиционированные элементы (абсолютно, липко, фиксированно)
- строчный
- флексовый
