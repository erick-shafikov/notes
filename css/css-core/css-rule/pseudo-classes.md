...
- :active - при клике на элемент может быть как ссылка так и формы, поля формы. В момент между нажатием и отжатием элемента
...
- :any-link - для всех состояний ссылки, относится ко всем элементам a, area, или link
- :autofill - если значение в input взято из автозаполнения
...
- :first - представляя первую страницу документа при печати, [используется с @-правилом @page](../styles-managment.md#page)
...
- :first-of-type - находит первого потомка своего типа среди детей родителя, то есть первые типы тегов из всех дочерних, то есть первого ребенка другого типа
...
- :focus - для элементов форм, при фокусе. Никогда не удаляйте фокусный outline (видимый индикатор фокуса), не заменяя его фокусным контуром подходящим под требования
- :focus-visible - работает по разном для клавиатуры и мыши при фокусировке
...
- :future - для титров, которые идут после :current
...
- :indeterminate - для полей ввода, которые находятся в неопределенном состоянии
