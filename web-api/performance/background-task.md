# background task api (-s)

IdleDeadline - для работы с requestIdleCallback

Window.requestIdleCallback() позволяет выполнять код без потери в исполнении других задач

- пределе timeRemaining() === 50 мс
- избегать внесения в dom
- исполнения promise
