...
Делегирование события – прием при котором можно определить обработчик на родительский элемент, чтобы не добавлять одинаковый обработчик на множество вложенных элементов. Побочный эффект - медленная прокрутка, так как обрабатывается событие на любом взаимодействии с контейнером

## пример. работа с ячейками таблицы
...
## пример. скрывать элемент по щелчку
...
passive: true сигнализирует для addEventListener что обработчик не собирается выполнять preventDefault(). Это намекает браузеру, что вы все еще хотите прослушивать событие в основном потоке, но наборщик также может продолжить и составить новый кадр.
