...
для использования каскадных слоев. Определение без слоя переопределит слой, так как вынесется в последний слой.

```scss
// в данном примере color == rebeccapurple, хотя должен был быть green так как более специфичный
p {
  color: rebeccapurple;
}
