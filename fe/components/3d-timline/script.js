...
const runScript = () => {
  ...
  const slidesRoot =
    root.querySelector < HTMLDivElement > ".timeline3d__slides";
  const track = root.querySelector < HTMLDivElement > "#timeline3d-scrollbar";
  const thumb =
    root.querySelector < HTMLDivElement > "#timeline3d-scrollbar-thumb";
  const inner = root.querySelector < HTMLDivElement > "#scrollbar-inner";
  const categoryElements =
    root.querySelectorAll < HTMLDivElement > ".timeline3d__category-item";
  ...
  const frameMaxEl =
    root.querySelector < HTMLDivElement > "#timeline3d-frame-max";
    ...
    for (let pos = minPos; pos <= maxPosWithTail; pos += 1) {
      ...
      html.push(
        `<div class="slide" data-position="${pos}">
          ${
            hasCard
              ? '<div class="slide__card"><div class="slide__card-inner"></div></div>'
              : ""
          }
          ${
            hasTitle
