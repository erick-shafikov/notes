...
Указывает ожидание, которое должен выполнить сервер. Используется для обработки больших тел запросов. Единственно значение
...
В случае успеха - 100, в случае ошибки - 417
...
Условный запрос. Сервер на запрос с таким заголовком возвращает 200 если, если ресурс изменился после даты в If-Modified-Since header. Если не изменился то 304 с Last-Modified с предыдущей датой изменения. Применяется только для get и head запросов. Игнорируется если используется [If-None-Match](#if-none-match). Сигнатура - [даты](./res-headers.md#expires)
...
определяет источник scheme, hostname, port который инициировал запрос.Заголовок Origin похож на заголовок [Referer](#referer), но не раскрывает путь и может быть пустым. Предоставляет источник запроса, когда он нужен. Используется:
