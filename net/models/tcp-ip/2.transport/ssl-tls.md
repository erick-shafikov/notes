...
Это подтверждение подлинности сервера. Обеспечивается с помощью симметричного шифрования. С помощью открытого и закрытого ключа. Шифрование - закрытым ключом. Шифрование применяется на хеш передаваемых данных. Сравнивается хеш полученных данных и открытый ключ. Используется инфраструктура открытых ключей, через удостоверяющий центр. Удостоверяющие центры представляют иерархию. Бывают самоподписанные сертификаты

# TLS

оборачивает https, использует tcp протокол.

Состоит из двух уровней:

- Протокол записи (record protocol) - в него входят все вышестоящие. Формат:
- - Тип сообщения (незашифрованная часть)
- - версия (незашифрованная часть) - TLS 1.2 (значение всегда)
- - длина (незашифрованная часть)
- - само сообщение (шифруется только сообщение), в свою очередь состоит из следующих частей:
- - - тип
- - - сообщение
- - - заполнение
- Протокол вышестоящего уровня. в этот уровень входят протоколы:
- - handshake - позволяет создать соединение
- - alert - позволяет оповещать об ошибках виды:
- - - фатальные ошибки: ошибка mac, неизвестный удостоверяющий центр unknown_ca, Ошибка шифрования encrypt_er
- - - предупреждения: срок действия certificate_expired, сертификат отозван certificate_revoked, неизвестный формат сертификата unsupported_certificate
- - application data
- - change cipher

Требует создание сессии - набор шифров tls (криптографические алгоритм), разделяемые ключи, описание сессии (id, узел)

При установке обменивается клиент и сервер:

1. Клиент отправляет сообщение = ClientHello + шифры tls + client key_share состоит из:

- tls record layer (вложенный словарь):
- - content-type - handshake
- - version
- - length
- - handshake protocol:
- - - version
- cipher suits - набор шифров, один из них выберет сервер

2. Сервер отправляет ответ несколько сообщений = ServerHello, шифры TLS, server key_share, сертификат, CertificateVerify, Finished = хеш функция от сообщения клиента и сервера, формируются ключи симметричного шифрования. Может понадобиться сертификат клиента CertificateRequest (нечасто) состоит из:
...
3. Клиент проверяет сертификат сервера - проверяются подписи удост. центра, проверка домена, срок
4. Клиент отправляет session ticket - нужен для восстановления соединения в течении одного дня без предварительного обмена
5. Для завершения отправляется сообщение finished, что подтверждается установку соединения, состоит из двух частей:

- change cipher spec
- само сообщение finished - считаются хеш от всех сообщений идет сравнение

6. Передача данных

Второй вариант 0-RTT (route trip time) для повторных установок:

1. используется предыдущие ключи шифрования. Клиент отправляет сообщение = ClientHello + шифры tls + client key_share + pre_shared_key
2. сервер использует предыдущие mac-ключи + данные (которые нужно передать)

Разрыв соединения:

- удачное закрытие: отправка оповещения close_notify должны отправить обе стороны
- при ошибке: alert протокол
