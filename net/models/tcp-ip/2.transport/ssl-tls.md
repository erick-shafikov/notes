...
1. Клиент отправляет сообщение = ClientHello + шифры tls + client key_share
2. Сервер отправляет ответ = ServerHello, шифры TLS, server key_share, сертификат, CertificateVerify, Finished = хеш функция от сообщения клиента и сервера, формируются ключи симметричного шифрования. Может понадобиться сертификат клиента CertificateRequest (нечасто)
3. Клиент проверяет сертификат сервера - проверяются подписи удост. центра, проверка домена, срок
4. Для завершения отправляется сообщение finished, что подтверждается установку соединения
